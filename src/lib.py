...
def train_test_accuracy(model, X_train, Y_train, X_test, Y_test):
    train_results = model.evaluate(
        X_train, np.asarray(Y_train), verbose=0, batch_size=256)
    test_results = model.evaluate(
        X_test, np.asarray(Y_test), verbose=0, batch_size=256)
...
def report_nn(model, X_test, Y_test):
    ...
    # model.predict([val_X], batch_size=1024, verbose=0)
    ...
    report = classification_report(
        y_actuals.tolist(), y_preds.tolist(), target_names=target_names)

    print(report)

    print("accuracy: {:0.3f}".format(
        metrics.accuracy_score(y_actuals.tolist(), y_preds.tolist())))

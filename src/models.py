...
def GRU_model(embedding_matrix, vocab_len):
    emb_dim = embedding_matrix.shape[1]
    gru_model = Sequential()
    gru_model.add(Embedding(vocab_len, emb_dim,
...
def CNN_GRU_model(embedding_matrix, vocab_len):
    emb_dim = embedding_matrix.shape[1]
    cnn_gru_model = Sequential()
    cnn_gru_model.add(Embedding(vocab_len, emb_dim,
                      trainable=False, weights=[embedding_matrix]))
    cnn_gru_model.add(
...
def CNN_LSTM_model(embedding_matrix, vocab_len):
    emb_dim = embedding_matrix.shape[1]
    cnn_lstm_model = Sequential()
    cnn_lstm_model.add(Embedding(vocab_len, emb_dim,
                       trainable=False, weights=[embedding_matrix]))
    cnn_lstm_model.add(
        Conv1D(64, kernel_size=3, padding='same', activation='relu'))
...
def BID_LSTM_model(embedding_matrix, vocab_len, emb_dim):
    lstmbd_model = Sequential()
    lstmbd_model.add(Embedding(vocab_len, emb_dim,
                     trainable=False, weights=[embedding_matrix]))
    ...
    lstmbd_model.compile(loss='categorical_crossentropy',
                         optimizer='adam', metrics=['accuracy'])
    lstmbd_model.summary()
    return (lstmbd_model)


def BID_GRU_model(embedding_matrix, vocab_len, emb_dim):
    grubd_model = Sequential()
    grubd_model.add(Embedding(vocab_len, emb_dim,
                    trainable=False, weights=[embedding_matrix]))
    ...
    grubd_model.compile(loss='categorical_crossentropy',
                        optimizer='adam', metrics=['accuracy'])
    grubd_model.summary()
    return (grubd_model)
